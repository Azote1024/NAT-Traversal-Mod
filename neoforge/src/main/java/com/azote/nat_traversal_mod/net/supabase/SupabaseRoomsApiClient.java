...
final class SupabaseRoomsApiClient {
    ...
    static Optional<String> fetchOpenRoomBody(String supabaseUrl, String supabaseKey, String roomName) {
        ...
        HttpRequest request = SupabaseRestClient.buildAuthorizedGetJsonRequest(
                endpoint,
                supabaseKey,
        ...
        try {
            ...
            SupabaseRestClient.ResponseCategory category = SupabaseRestClient.classifyDefault(statusCode);
            if (category != SupabaseRestClient.ResponseCategory.SUCCESS) {
                if (statusCode == 404) {
                    NatTraversalMod.LOGGER.warn(
                            "[nat-traversal-mod] Supabase rooms endpoint unavailable. status={}, room_name='{}'. Fallback to original target.",
                            statusCode,
                            roomName
                    );
